...
class PixPaymentJwtBinderSpec extends Specification {

    def "If the body has already been decoded we don't bind"() {

        given:
        def payment = TestRequestDataFactory.testPixPayment()
        MicronautAwsProxyRequest request = Mock(MicronautAwsProxyRequest)
        request.isBodyDecoded() >> true
        request.getBody(CreatePixPayment) >> Optional.of(payment)
        request.getContentType() >> MediaType.APPLICATION_JSON_TYPE
        PixPaymentJwtBinder binder = new PixPaymentJwtBinder(new ObjectMapper())
    ...
    def "If the body has not already been decoded we bind"() {
        ...
        PixPaymentJwtBinder binder = new PixPaymentJwtBinder(mapper)
