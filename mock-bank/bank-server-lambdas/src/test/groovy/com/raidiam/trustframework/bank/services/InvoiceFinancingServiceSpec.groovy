...
class InvoiceFinancingServiceSpec extends CleanupSpecification {
    ...
    def setup() {
        if (runSetup) {
            ...
            consentPermissionsRepository.save(aConsentPermission(CreateConsentData.PermissionsEnum.INVOICE_FINANCINGS_READ, testConsent.getConsentId()))
            consentPermissionsRepository.save(aConsentPermission(CreateConsentData.PermissionsEnum.INVOICE_FINANCINGS_WARRANTIES_READ, testConsent.getConsentId()))
            consentPermissionsRepository.save(aConsentPermission(CreateConsentData.PermissionsEnum.INVOICE_FINANCINGS_SCHEDULED_INSTALMENTS_READ, testConsent.getConsentId()))
            consentPermissionsRepository.save(aConsentPermission(CreateConsentData.PermissionsEnum.INVOICE_FINANCINGS_PAYMENTS_READ, testConsent.getConsentId()))
            ...
            testContract = testEntityDataFactory.createAndSaveFullContract(
                    ...
                    EnumContractProductTypeInvoiceFinancings.DESCONTADOS.toString(),
    ...
    def "We can get a contract and check its values"() {
        when:
        def savedContractOptional =  contractsRepository.findByContractIdAndContractType(testContract.getContractId(), AccountOrContractType.INVOICE_FINANCING.name())
        ...
        def newContractPage = invoiceFinancingService.getInvoiceFinancingContract(testConsent.getConsentId().toString(), savedContract.getContractId())
        ...
        newContract.getCET() == savedContract.getCet()
        ...
        newContract.getInterestRates() == List.of(savedInterestRates.getInvoiceFinancingsDTO())
        newContract.getContractedFees() == List.of(savedContractedFees.getInvoiceFinancingsDTO())
        newContract.getContractedFinanceCharges() == List.of(savedFinanceCharges.getInvoiceFinancingsDTO())
    }


    def "We can get all contracts and check their values"() {
        when:
        def page = invoiceFinancingService.getInvoiceFinancingContractList(Pageable.unpaged(), testConsent.getConsentId())

        then:
        page.getData().size() == 1
        page.getData().get(0) == testContract.getInvoiceFinancingsDTOList()
    }
    ...
    def "We can get the Warranty information for a contract"() {
        ...
        when:
        def contractWarrantyDTO = warranties.first().getInvoiceFinancingDTO()
        def foundWarranties = invoiceFinancingService.getInvoiceFinancingWarranties(Pageable.unpaged(), testConsent.getConsentId().toString(), testContract.getContractId())
    ...
    def "We can get the scheduled instalments for a contract"() {
        when:
        def instalments = invoiceFinancingService.getInvoiceFinancingScheduledInstalments(testConsent.getConsentId().toString(), testContract.getContractId())
    ...
    def "We can get the payments information for a contract"() {
        when:
        def paymentsResponse = invoiceFinancingService.getInvoiceFinancingPayments(testConsent.getConsentId().toString(), testContract.getContractId())
    ...
    def "we cannot get a response when the consent not have permissions"() {
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingContract(testConsent2.getConsentId(), testContract2.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingScheduledInstalments(testConsent2.getConsentId(), testContract2.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingWarranties(Pageable.unpaged(), testConsent2.getConsentId(), testContract2.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingPayments(testConsent2.getConsentId(), testContract2.getContractId())
    ...
    def "we cannot get a response when the consent owner is not the contract owner"() {
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingContract(testConsent.getConsentId(), testContract2.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingScheduledInstalments(testConsent.getConsentId(), testContract2.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingWarranties(Pageable.unpaged(), testConsent.getConsentId(), testContract2.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingPayments(testConsent.getConsentId(), testContract2.getContractId())
    ...
    def "we cannot get response without authorised status"() {
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingContract(testConsent.getConsentId(), testContract.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingPayments(testConsent.getConsentId(), testContract.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingScheduledInstalments( testConsent.getConsentId(), testContract.getContractId())
        ...
        when:
        invoiceFinancingService.getInvoiceFinancingWarranties(Pageable.unpaged(),testConsent.getConsentId(), testContract.getContractId())
