...
(async () => {
  ...
  async function setupClient(bank) {
    ...
    const DirectoryFAPIClient = directoryIssuer.FAPIClient;
    ...
    directoryFapiClient[custom.http_options] = function (options) {
      ...
      options.https.certificateAuthority = fs.readFileSync(
        certsPath + 'ca.pem'
    ...
    try {
      ...
      fapiClient = await FAPIClient.register(config.data.client, {
        jwks: keyset,
      });
