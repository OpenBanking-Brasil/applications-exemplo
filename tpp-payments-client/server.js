...
(async () => {
  ...
  const availableBanks = TotalBanks.filter((e) =>
    e.AuthorisationServers.some((as) =>
      as.ApiResources.some(
        (apifamily) => apifamily.ApiFamilyType == "payments-consents"
      )
    )
  );
  ...
  async function setupClient(bank) {
    ...
    localIssuer.metadata.token_endpoint_auth_methods_supported.includes(
      ...
      ? (config.data.client.token_endpoint_auth_method =
          process.env.PREFERRED_TOKEN_AUTH_MECH)
    ...
    localIssuer.metadata.request_uri_parameter_supported
      ? (config.data.client.require_pushed_authorization_requests = true)
      : (config.data.client.require_pushed_authorization_requests = false);
    dcrLog(
      "Use pushed authorisation requests if the bank supports it. Will use PAR? %O",
      config.data.client.require_pushed_authorization_requests
    );
    ...
    dcrLog(
      ...
      config.data.client.software_statement
    );
  ...
  app.post("/payment", async (req, res) => {
    ...
    consentLog(
      "Sending customer to select the bank they want to make the payment from"
    );
    res.render("idp", {});
  });
